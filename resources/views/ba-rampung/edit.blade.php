...
<form method="POST" action="{{ route('ba-rampung.update', $baRampung) }}"
      x-data="baForm({
    ...
    <div class="card p-6">
        <h3 class="font-semibold text-gray-900 mb-5">📄 1. Data BA Rampung — {{ $baRampung->nomor_ba }}</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                <label class="label">Nomor BA</label>
                <input type="text" disabled value="{{ $baRampung->nomor_ba }}" class="input bg-gray-50 text-gray-500">
            </div>
            ...
            <div>
                ...
                <input type="text" name="nomor_mo" value="{{ old('nomor_mo', $baRampung->nomor_mo) }}" required class="input">
            </div>
            <div>
                <label class="label">Nomor Purchase Order (PO)</label>
                <input type="text" name="nomor_po" value="{{ old('nomor_po', $baRampung->nomor_po) }}" required class="input">
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr>
                        ...
                        <td class="px-4 py-3 w-40"><input type="number" step="0.01" min="0.01" name="kuantum_gabah" x-model.number="gabah" required class="input"></td>
                        <td class="px-4 py-3 font-medium text-gray-700">Beras (HGL)</td>
                        <td class="px-4 py-3 w-40"><input type="number" step="0.01" min="0" name="kuantum_beras" x-model.number="beras" required class="input"></td>
                        <td class="px-4 py-3 text-gray-500" x-text="rendemen(beras) + ' %'"></td>
                    </tr>
                    <tr>
                        <td colspan="2"></td>
                        <td class="px-4 py-3 font-medium text-gray-700">Menir</td>
                        <td class="px-4 py-3 w-40"><input type="number" step="0.01" min="0" name="kuantum_menir" x-model.number="menir" class="input"></td>
                        <td class="px-4 py-3 text-gray-500" x-text="rendemen(menir) + ' %'"></td>
                    </tr>
                    <tr>
                        <td colspan="2"></td>
                        <td class="px-4 py-3 font-medium text-gray-700">Bekatul</td>
                        <td class="px-4 py-3 w-40"><input type="number" step="0.01" min="0" name="kuantum_bekatul" x-model.number="bekatul" class="input"></td>
                ...
                <div>
                    ...
                    <select name="gudang_id" required class="input">
                        @foreach ($gudangs as $g)
                            <option value="{{ $g->id }}" @selected(old('gudang_id', $baRampung->gudang_id) == $g->id)>{{ $g->nama_gudang }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="mitra_pengolahan_id" required class="input">
                        @foreach ($mitras as $m)
                            <option value="{{ $m->id }}" @selected(old('mitra_pengolahan_id', $baRampung->mitra_pengolahan_id) == $m->id)>{{ $m->nama_mitra }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="status_pbp" class="input">
                        @foreach (\App\Models\BaRampung::STATUS_PBP as $val => $label)
                            <option value="{{ $val }}" @selected(old('status_pbp', $baRampung->status_pbp) === $val)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="card p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input list="pegawai-list" name="nama_penandatangan" value="{{ old('nama_penandatangan', $baRampung->nama_penandatangan) }}" required class="input">
                ...
                <div>
                    ...
                    <input type="text" name="jabatan_penandatangan" value="{{ old('jabatan_penandatangan', $baRampung->jabatan_penandatangan) }}" required class="input">
                </div>
                ...
                <div>
                    ...
                    <select name="pimpinan_cabang_id" required class="input">
                        @foreach ($pimpinans as $p)
                            <option value="{{ $p->id }}" @selected(old('pimpinan_cabang_id', $baRampung->pimpinan_cabang_id) == $p->id)>{{ $p->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <div class="card p-6">
        ...
        <textarea name="catatan" rows="2" class="input">{{ old('catatan', $baRampung->catatan) }}</textarea>
    </div>

    <div class="flex items-center justify-between">
        <a href="{{ route('ba-rampung.show', $baRampung) }}" class="btn-secondary">← Kembali</a>
        <div class="flex gap-3">
            <button type="submit" name="action" value="draft" class="btn-secondary">💾 Simpan sebagai Draft</button>
            <button type="submit" name="action" value="submit" class="btn-primary">✅ Simpan &amp; Kirim Verifikasi</button>
        </div>
    </div>
</form>
