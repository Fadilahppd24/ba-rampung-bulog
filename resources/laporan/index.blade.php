...
<div class="card p-5">
    ...
    <div class="mb-5">
        ...
        @if ($isAdminGudang)
            <p class="text-sm text-gray-500 mt-1">
                Laporan khusus gudang:
                <span class="font-medium text-bulog-800">
                    {{ $gudangUser?->nama_gudang ?? '-' }}
                </span>
            </p>
        @else
            <p class="text-sm text-gray-500 mt-1">
                Pilih jenis laporan dan filter data yang ingin ditampilkan.
            </p>
        @endif
    </div>
    ...
    <form method="GET" action="{{ route('laporan.index') }}" class="space-y-4">
        ...
        {{-- Jenis Laporan --}}
        <div>
            <label class="label mb-2 block">Jenis Laporan</label>
                ...
                @foreach ($jenisList as $val => $label)
                    <label class="cursor-pointer">
                        ...
                        <div class="rounded-xl border-2 border-gray-200
                            ...
                            text-gray-600 peer-checked:text-bulog-800
                            transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            @if ($isAdminGudang)
                ...
                <div>
                    <label class="label mb-1 block">Gudang</label>

                    <div class="input bg-gray-100 text-gray-700 cursor-not-allowed">
                        {{ $gudangUser?->nama_gudang ?? 'Gudang tidak ditemukan' }}
                    </div>

                    {{-- Tetap kirim gudang_id ke controller --}}
                    <input
                        type="hidden"
                        name="gudang_id"
                        value="{{ $gudangUser?->id }}"
            ...
            {{-- Mitra --}}
            <div>
                <label class="label mb-1 block">Mitra Pengolahan</label>
            ...
            {{-- Bulan --}}
            <div>
                <label class="label mb-1 block">Bulan</label>
            ...
            {{-- Tahun --}}
            <div>
                <label class="label mb-1 block">Tahun</label>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-5 pb-0">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">
                    Menampilkan {{ $rows->count() }} baris data
                </p>
            </div>
